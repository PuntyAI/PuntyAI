...
class TestFacebookDeliveryUpdate:
    """Test FacebookDelivery.post_update() — standalone live updates."""

    @pytest.mark.asyncio
    async def test_post_update_to_page(self):
        from punty.delivery.facebook import FacebookDelivery
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"id": "123456_update1"}
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            result = await fb.post_update("Big win celebration!")

        assert result["status"] == "posted"
        assert result["post_id"] == "123456_update1"
        # Verify URL is page feed (not comments)
        call_args = mock_client.post.call_args
        assert "123456/feed" in call_args[0][0]

    @pytest.mark.asyncio
    async def test_post_update_raises_if_not_configured(self):
        from punty.delivery.facebook import FacebookDelivery
        ...
        with pytest.raises(ValueError, match="Facebook not configured"):
            await fb.post_update("Hello")

    @pytest.mark.asyncio
    async def test_post_update_raises_on_api_error(self):
        from punty.delivery.facebook import FacebookDelivery
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            with pytest.raises(ValueError, match="Facebook post error"):
                await fb.post_update("Hello")


class TestFacebookDeliveryComment:
    """Test FacebookDelivery.post_comment() — requires pages_manage_engagement."""
