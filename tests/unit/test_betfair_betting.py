...
class TestRefreshBetSelections:
    ...
    def _setup_db_responses(self, mock_db, queued_bets, setting_value, picks, runners,
        ...
        Call order in refresh_bet_selections (gates removed):
        1. select(BetfairBet) → queued bets
        Then per bet:
        2. select(count(Runner)) → NTD runner count check
        ...
        mock_db.execute = AsyncMock(
            side_effect=[bets_result,
                         runner_count_result, picks_result]
...
class TestCycleBetSelection:
    ...
    async def test_cycle_to_next_pick(self, mock_db):
        ...
        # Call order: select bet, select picks, runner checks (no settings queries)
...
class TestCalibration:
    ...
    def test_pp_floor_rejects_low_calibrated(self):
        ...
        from punty.betting.calibration import calibrate_probability
        cal_map = {7: 0.53}  # Predicted 75% → actual 53%
        calibrated = calibrate_probability(0.75, cal_map)
        assert calibrated == 0.53
        # With a 0.55 floor, this bet should be rejected (calibrated 53% < 55%)
